    ...
    const handleSubmit = async () => {
        files.forEach((fileData) => {
            if (!fileData.title.trim()) {
                alert('모든 파일에 제목을 입력해주세요.');
                throw new Error('Title is required for all files');
            }
        });
        ...
        <>
            {loading && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center">
                    ...
                    <div className="font-neodgm font-bold text-white text-2xl animate-pulse">노예 일상 추가 중...</div>
                </div>
            )}
            ...
            <div className="flex flex-col w-full h-full p-4 md:p-6 gap-6">
                ...
                <div className="relative w-full max-w-4xl mx-auto flex-shrink-0">
                    ...
                    >
                        <div className="text-center font-esamanru font-bold text-[#5c2500] px-4">
                            <p className="text-xl md:text-2xl mb-2">파일을 드래그하거나 클릭하세요</p>
                            <p className="text-base md:text-base opacity-70">이미지 / 비디오 파일만 업로드</p>
                            <p className="text-base md:text-base opacity-70">@ 100MB 이하 / 최대 100개</p>
                        </div>
                    </div>
                </div>
                ...
                {files.length > 0 && (
                    <div onClick={() => handleSubmit()} className="relative w-full max-w-4xl mx-auto flex-shrink-0">
                        ...
                        <button
                            disabled={loading}
                            className="w-full py-4 md:py-5 bg-box-background font-bold font-neodgm text-[#5c2500] text-lg md:text-xl lettering-widest"
                        >
